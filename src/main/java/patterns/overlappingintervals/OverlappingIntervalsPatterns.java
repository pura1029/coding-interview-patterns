...
public class OverlappingIntervalsPatterns {
    ...
    /** Merge Intervals. Sort by start, extend end. */
    ...
    /** Check if Intervals Overlap. Sort and compare adjacent. */
    public static boolean hasOverlap(int[] a, int[] b) { return a[0] < b[1] && b[0] < a[1]; }
    /** Remove Covered Intervals. Sort, skip fully covered. */
    ...
    /** Summary Ranges. Group consecutive numbers. */
    ...
    /** Minimum Changes to Make Alternating Binary String. Count mismatches for both patterns. */
    ...
    /** Can Place Flowers. Check empty neighbors. */
    ...
    /** Count Days Between Two Dates (simplified). Date arithmetic. */
    ...
    /** Range of Each Time Interval. Compute end - start for each. */
    public static int[] intervalLengths(int[][] iv) { int[] r = new int[iv.length]; for (int i = 0; i < iv.length; i++) r[i] = iv[i][1]-iv[i][0]; return r; }
    /** Determine If Two Events Have Conflict. Two events overlap if start < other end. */
    ...
    /** Interval List Intersections. Two-pointer on sorted lists. */
    ...
    /** Minimum Number of Arrows to Burst Balloons. Sort by end, count non-overlapping. */
    ...
    /** Car Pooling. Sweep line on pickup/dropoff. */
    ...
    /** My Calendar I (no double booking). TreeMap for overlap detection. */
    ...
    /** Add Bold Tag in String. Merge bold ranges, wrap. */
    ...
    /** Divide Intervals Into Minimum Groups. Sweep line or min-heap. */
    ...
    /** Minimum Interval to Include Each Query. Sort queries + intervals, sweep. */
    ...
    /** Maximum Number of Events That Can Be Attended. Greedy: earliest ending first. */
    ...
    /** Meeting Rooms II (min rooms). Min-heap of end times. */
    ...
    /** Employee Free Time. Merge all schedules, find gaps. */
    ...
    /** Skyline Problem (simplified output). Sweep line with max-heap of heights. */
    ...
    /** Data Stream as Disjoint Intervals. TreeMap for interval merging. */
    ...
    /** My Calendar III (max concurrent events). Sweep line counting overlaps. */
    ...
    /** Range Module. TreeMap for tracked ranges. */
    ...
    /** Minimum Number of Taps to Open Water. Interval coverage greedy. */
    ...
    /** Count Ways to Group Overlapping Ranges. Merge + count independent groups. */
    ...
    /** Maximum CPU Load. Sweep line for peak usage. */
    ...
    /** Points That Intersect With Cars. Merge intervals, count total points. */
