...
public class DynamicProgrammingPatterns {
    ...
    /** Maximum Subarray (Kadane's). Extend or restart at each element. */
    ...
    /** Is Subsequence. Two-pointer or DP matching. */
    ...
    /** Tribonacci Number. dp[i] = dp[i-1] + dp[i-2] + dp[i-3]. */
    ...
    /** Pascal's Triangle. dp[r][c] = dp[r-1][c-1] + dp[r-1][c]. */
    ...
    /** Longest Increasing Subsequence. DP or binary search patience sort. */
    ...
    /** 0/1 Knapsack. Include/exclude item, 1D rolling array. */
    ...
    /** Longest Common Subsequence. 2D DP: match or max(skip one). */
    ...
    /** Word Break. dp[i] = any dp[j] && word[j.i] in dict. */
    ...
    /** House Robber II (circular). Two passes: skip first or last. */
    ...
    /** Decode Ways. dp[i] based on 1-digit and 2-digit splits. */
    ...
    /** Partition Equal Subset Sum. 0/1 knapsack for target = sum/2. */
    ...
    /** Regular Expression Matching. DP on pattern with '.' and '*'. */
    ...
    /** Longest Increasing Path in Matrix. DFS + memo on each cell. */
    ...
    /** Burst Balloons. Interval DP: choose last balloon to pop. */
    ...
    /** Minimum Cost to Cut a Stick. Interval DP on cut positions. */
    ...
    /** Wildcard Matching. DP on '?' (any char) and '*' (any seq). */
    ...
    /** Interleaving String. 2D DP: s1[i] or s2[j] matches s3[i+j]. */
    ...
    /** Maximum Profit in Job Scheduling. Sort by end, binary search + DP. */
    ...
    /** Palindrome Partitioning II. Min cuts: precompute palindrome table. */
